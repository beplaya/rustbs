...
impl Babysitter {
    fn get_earnings(&self, arrival_time: i32, departure_time: i32, bedtime: i32) -> i32 {
        ...
        if arrival_time != departure_time {
            earnings += self.rate.standard_rate * self.get_hours_before_bedtime(arrival_time, departure_time, bedtime);
            earnings += self.rate.house_sit_rate * self.get_hours_after_bedtime(arrival_time, departure_time, bedtime);
            earnings += self.rate.after_midnight_bonus * self.get_hours_after_midnight(arrival_time, departure_time);
        }
        return earnings;
    }
    ...
    fn all_work_after_midnight(&self, arrival_time: i32, departure_time: i32) -> bool {
        return arrival_time < self.schedule.earliest_arrival && departure_time < self.schedule.earliest_arrival;
    }
    ...
    fn get_hours_before_bedtime(&self, arrival_time: i32, departure_time: i32, bedtime: i32) -> i32 {
        ...
        if self.is_zero_hour_shift(arrival_time, departure_time) {
            hours = 0;
        } else if arrival_time < self.schedule.earliest_arrival {
            hours = 0;
        } else {
    ...
    fn get_hours_after_bedtime(&self, arrival_time: i32, departure_time: i32, bedtime: i32) -> i32 {
        ...
        } else if departure_time < self.schedule.earliest_arrival {
            hours = (self.schedule.latest_bedtime - bedtime) + departure_time;
        } else {
...
mod babysitter_tests {
    ...
    fn it_gets_hours_of_work_before_bedtime() {
        assert_eq!(BABYSITTER.get_hours_before_bedtime(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end, SCHEDULE.latest_bedtime), SCHEDULE.latest_bedtime - SCHEDULE.earliest_arrival);
        assert_eq!(BABYSITTER.get_hours_before_bedtime(SCHEDULE.latest_shift_end, SCHEDULE.latest_shift_end, SCHEDULE.latest_bedtime), SCHEDULE.latest_shift_end - SCHEDULE.latest_shift_end);
        assert_eq!(BABYSITTER.get_hours_before_bedtime(1, SCHEDULE.latest_shift_end, SCHEDULE.latest_bedtime), 0);
        assert_eq!(BABYSITTER.get_hours_before_bedtime(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end, SCHEDULE.earliest_arrival), 0);

        assert_eq!(BABYSITTER.get_hours_before_bedtime(SCHEDULE.latest_shift_end - 2, SCHEDULE.latest_shift_end - 1, SCHEDULE.latest_bedtime - 4), 0);
    }

    #[test]
    fn it_gets_hours_of_work_after_bedtime() {
        assert_eq!(BABYSITTER.get_hours_after_bedtime(SCHEDULE.earliest_arrival, SCHEDULE.latest_bedtime, 8), 4);
        assert_eq!(BABYSITTER.get_hours_after_bedtime(SCHEDULE.latest_shift_end, SCHEDULE.latest_shift_end, SCHEDULE.latest_bedtime), 0);
        assert_eq!(BABYSITTER.get_hours_after_bedtime(1, SCHEDULE.latest_shift_end, SCHEDULE.latest_bedtime), 3);
        assert_eq!(BABYSITTER.get_hours_after_bedtime(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end, SCHEDULE.earliest_arrival), (SCHEDULE.latest_bedtime - SCHEDULE.earliest_arrival) + SCHEDULE.latest_shift_end);

        assert_eq!(BABYSITTER.get_hours_after_bedtime(SCHEDULE.latest_shift_end - 2, SCHEDULE.latest_shift_end - 1, SCHEDULE.latest_bedtime - 4), 1);
    }

    #[test]
    fn it_gets_hours_of_work_after_midnight() {
        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.earliest_arrival, SCHEDULE.latest_bedtime), 0);
        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end), SCHEDULE.latest_shift_end);
        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end), SCHEDULE.latest_shift_end);
        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end), SCHEDULE.latest_shift_end);
        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.earliest_arrival, SCHEDULE.latest_shift_end), SCHEDULE.latest_shift_end);

        assert_eq!(BABYSITTER.get_hours_after_midnight(SCHEDULE.latest_shift_end - 2, SCHEDULE.latest_shift_end - 1), 1);
    }
